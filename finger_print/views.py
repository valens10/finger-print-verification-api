...
from .fingerprint_scanner import read_file, handle_verification,delete_fingerprint_file, THRESHOLD_RATE, handle_create_enrollement, \
                handle_start_enrollment, handle_add_to_enrollment, handle_finish_enrollement, DPFJ_E_MORE_DATA, handle_generate_finger_print_template, generate_random_filename
import ctypes
from .serializers import FingerprintSerializer


@api_view(['POST'])
def enroll_user_fingerprint(request):
    user_id = request.data.get('user_id')
    created_by = request.data.get('created_by')
    finger_print_file = request.data.get('finger_print_file')

    if not user_id or not finger_print_file or not created_by:
        return Response({"detail": "user_id, created_by and finger_print_file are required"}, status=400)
    ...
    try:
        ...
        fingerprint_instance, created = Fingerprint.objects.update_or_create(
            user_id=user_id, created_by=created_by, defaults={"template": bytes(data2)}
        )
        res_data = {
            "message": "Fingerprint enrolled successfully",
            'is_more_data': False,
            "user_id": user_id,
            "id": fingerprint_instance.id
            }
        
        # Delete the saved fingerprint file
        delete_fingerprint_file(file_path)
        handle_finish_enrollement()
...
@api_view(['POST'])
def enroll_client_fingerprint(request):
    client_id = request.data.get('client_id')
    created_by = request.data.get('created_by')
    finger_print_file = request.data.get('finger_print_file')

    if not client_id or not finger_print_file or not created_by:
        return Response({"detail": "client_id, created_by and finger_print_file are required"}, status=400)
    ...
    try:
        ...
        # Store the fingerprint in the database
        fingerprint_instance, created = Fingerprint.objects.update_or_create(
            client_id=client_id, is_client=True,created_by=created_by, defaults={"template": bytes(data2)}
        )
        res_data = {
            "message": "Fingerprint enrolled successfully",
            'is_more_data': False,
...
@api_view(['POST'])
def verify_user_fingerprint(request):
    """
    ...
    try:
        ...
        if not user_id or not finger_print_file:
            return Response({"detail": "user_id and fingerprint file are required"}, status=400)
...
@api_view(['POST'])
def verify_client_fingerprint(request):
    """
    Verifies the fingerprint against the stored template.
    """
    try:
        ...
        if not client_id or not finger_print_file:
            return Response({"detail": "client_id and fingerprint file are required"}, status=400)
        ...
        # Retrieve stored fingerprint from database
        fingerprint_instance = Fingerprint.objects.get(client_id=client_id, is_client=True)
        stored_template = fingerprint_instance.template  # Stored as binary data
        size2 = len(stored_template)
        ...
        return Response({"detail": "No fingerprint found for this client"}, status=404)
...
#Get user data API by user ID
@api_view(['GET'])
def get_user_fingerprint_data(request, user_id):
    try:
        user_instance = Fingerprint.objects.filter(user_id=user_id).first()
        
        user_data = FingerprintSerializer(user_instance).data
...
@api_view(['GET'])
def get_client_fingerprint_data(request, client_id):
    try:

        client_instance = Fingerprint.objects.filter(client_id=client_id).first()
        user_data = FingerprintSerializer(client_instance).data
